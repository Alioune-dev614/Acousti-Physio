...
def save_as_csv(header, data_list_csv):
    with open('mesure_fft_amelie.csv', 'w', encoding='UTF8', newline='') as f:
        ...
        # write multiple rows
        writer.writerows(data_list_csv)
...
def fft_data_mot(txt_file, sub_dir_prime, j, phrase, mot, locuteur, graph_path, preop_ou_postop):  
    ...
    for son in info_sons:
        ...
        data_to_add_in_csv = []
        data_to_add_in_csv.append(locuteur)
        data_to_add_in_csv.append(preop_ou_postop)
        data_to_add_in_csv.append(strip_accents(phrase))
        data_to_add_in_csv.append(strip_accents(mot))
        
        position_mot_dans_phrase = j.split("_", 1)[0]
        data_to_add_in_csv.append(int(position_mot_dans_phrase))
        ...
        son[2] = ''.join([i for i in son[2] if not i.isdigit()])
        
        data_to_add_in_csv.append(son[2])
        
        regroupement, position_dans_regroupement = trouver_regroupement(info_sons, info_sons.index(son))
        
        data_to_add_in_csv.append(str(regroupement))
        data_to_add_in_csv.append(str(position_dans_regroupement))
        ...
        try:
            ...
            for peak_ind in peaks[0][:4]:
                ...
                data_to_add_in_csv.append(py)
        except:
            data_to_add_in_csv.append(" ")
        
        # diff
        try:
            peaks = find_peaks(AN_liss_y[1000:], prominence=5, distance=1250)
            for i in range(3):                
                py1 = AN_liss_y[peaks[0][i] +1000]
                py2 = AN_liss_y[peaks[0][i+1] +1000]
                data_to_add_in_csv.append(np.abs(py1 - py2))
        except:
            pass
            
        # moyenne
        try :
            data_to_add_in_csv.append(np.mean(AN_dBMag[:110]))
        except:
            data_to_add_in_csv.append("")
            print(" - ", mot, " pas de AN dBMag !")
        ...
        try:
            peaks = find_peaks(WAV_liss_y[1000:], prominence=5, distance=1250)
            for i in range(3):                
                py1 = WAV_liss_y[peaks[0][i] +1000]
                py2 = WAV_liss_y[peaks[0][i+1] +1000]
                data_to_add_in_csv.append(np.abs(py1 - py2))
        except:
            pass
        ...
        # moyenne
        try:
            data_to_add_in_csv.append(np.mean(WAV_dBMag[:110]))
        except:
            data_to_add_in_csv.append("")
            print(" - ", mot, " pas de WAV dBMag !")
        
        ## sup 
        try:
            if np.mean(WAV_liss_y[:110]) > np.mean(AN_liss_y[:110]):
                data_to_add_in_csv.append("WAV")
            if np.mean(WAV_liss_y[:110]) < np.mean(AN_liss_y[:110]):
                data_to_add_in_csv.append("AN")
            if np.mean(WAV_liss_y[:110]) == np.mean(AN_liss_y[:110]):
                data_to_add_in_csv.append("egal")
        except:
            data_to_add_in_csv.append("")
        
        
        data_list_csv.append(data_to_add_in_csv)
    ...
    for dir_patient_temoins in os.listdir(os.path.join(path, locuteur)):
        ...
        if "FINAL" in dir_patient_temoins or "postop" in dir_patient_temoins or "preop" in dir_patient_temoins:
            ...
            if "postop" in dir_patient_temoins or "preop" in dir_patient_temoins:
                ...
                if "postop" in dir_patient_temoins:
                    preop_ou_postop = "postop"
                else:
                    preop_ou_postop = "preop"
                            ...
                            for mot in mots:
                                ...
                                for fichier in os.listdir(sub_dir_prime):
                                    ...
                                    if ".TextGrid" in fichier and ("_" + mot2) in fichier2:
                                        textgrid = fichier
                                        data_list_csv = data_list_csv + fft_data_mot(path_txt, sub_dir_prime, textgrid, phrase_corrigee, mot, locuteur, graph_directory, preop_ou_postop)
...
def fft_data_all_locuteur():
    ...
    header = ["locuteur", "preop ou postop", "phrase", "mot", "position du mot dans la phrase", "son", "position son dans le mot", "position dans le regroupement", "AN pic1", "AN pic2", "AN pic3", "AN pic4", "AN diff pic1 pic2", "AN diff pic2 pic3", "AN diff pic3 pic4", "AN spectre moyen", "WAV pic1", "WAV pic2", "WAV pic3", "WAV pic4", "WAV diff pic1 pic2", "WAV diff pic2 pic3", "WAV diff pic3 pic4", "WAV spectre moyen", "spectre sup"]
    ...
    for locuteur in os.listdir(path):
        ...
        if '.' not in locuteur:
            ...
            print(locuteur)        
            data_list_csv = data_list_csv + fft_data_locuteur(path, locuteur)
            
    save_as_csv(header, data_list_csv)
